...
const hasValidationErrors =
  imageRecords.length !== heritageSites.length ||
  missingImageFiles.length > 0 ||
  duplicates(heritageSites.map((site) => site.id)).length > 0 ||
  duplicates(heritageSites.map((site) => site.slug)).length > 0 ||
  invalidCoordinates.length > 0 ||
  heritageSites.some((site) => !site.sourceUrl) ||
  heritageSites.some((site) => site.shortDescription.length < 40);

if (hasValidationErrors) process.exitCode = 1;
